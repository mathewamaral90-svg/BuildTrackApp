...
export default function Community(){
 const [builds,setBuilds]=useState<Build[]>([]),[posts,setPosts]=useState<Post[]>([]),[show,setShow]=useState(false),[error,setError]=useState(''),[loading,setLoading]=useState(true),[query,setQuery]=useState('')
 async function load(q=''){setLoading(true);const r=await fetch(q?'/api/discover?q='+encodeURIComponent(q):'/api/community');const d=await r.json();if(!r.ok)throw Error(d.error);setBuilds(d.builds);if(!q)setPosts(d.posts);setLoading(false)}
 async function search(e:FormEvent<HTMLFormElement>){e.preventDefault();setError('');try{await load(query)}catch(e){setError(e instanceof Error?e.message:'Search failed');setLoading(false)}}
