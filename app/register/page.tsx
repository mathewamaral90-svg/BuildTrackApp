...
export default function Register(){
 ...
 return <main className="auth-wrap"><div className="auth-card"><Link className="small-link" href="/">← BuildTrack</Link><h1>Create your garage</h1><p className="muted">Create your account and start documenting your builds.</p><form className="form" onSubmit={submit}>{error&&<div className="error">{error}</div>}<label>Display name<input name="displayName" required maxLength={60}/></label><label>Username<input name="username" required pattern="[A-Za-z0-9_]{3,24}" placeholder="evo_mick"/></label><label>Email <span aria-hidden="true">*</span><input name="email" type="email" required autoComplete="email" placeholder="you@example.com"/></label><label>Password<input name="password" type="password" minLength={8} required autoComplete="new-password"/></label><button className="btn btn-primary" disabled={loading}>{loading?'Creating…':'Create account'}</button></form><p className="muted">Already have an account? <Link className="small-link" href="/login">Log in</Link></p></div></main>
}