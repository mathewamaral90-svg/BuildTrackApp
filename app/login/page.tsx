...
export default function Login(){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){
  ...
  try{const f=new FormData(e.currentTarget); const r=await fetch('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({identifier:f.get('identifier'),password:f.get('password')})}); const d=await r.json().catch(()=>({}))
